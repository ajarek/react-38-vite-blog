...
export const Posts = () => {
  ...
  const commentsPost = data.comments.filter(
  ...
  return (
    <div className={'root'}>
      <div className={'posts-desc'}>
        ...
        <p className={'desc'}>{postsDesc.description}</p>
        <p>Date : {new Date(postsDesc.datePublished).toLocaleDateString()}</p>
        <h5>
          Author : {foundAuthor[0].firstName + ' ' + foundAuthor[0].lastName}
        </h5>
            ...
            >
              <img
                src={`https://joeschmoe.io/api/v1/${
                  data.authors.find((author) => author.id == el.authorId)
                    .firstName
                }`}
                alt=''
              />
